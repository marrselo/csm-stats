...
@Index('search_sal_orders', ['channel', 'warehouseName', 'warehouseAddress', 'comments'], {
	fulltext: true
})
...
export class SalOrders {
	...
	@Column('int', {
		name: 'user_id',
		nullable: true,
		comment: 'Campo de auditoria que almacena el id del empleado que registro el pedido',
		unsigned: true
	})
	userId: number | null
	...
	@Column('int', {
		...
		nullable: true,
		comment: 'Estados: 1. Pendiente, 2. En proceso, 3. Despachado, 4. No despachable',
		default: () => "'1'"
	})
	...
	@Column('int', {
		...
		comment: '1: Pendiente, 2: Aprobado, 3: Rechazado, 4: Aplicado, 5: No aplica',
		unsigned: true,
		default: () => "'5'"
	})
	...
	@Column('timestamp', {
		...
		nullable: true,
		comment: 'Fecha de creación del registro en la base de datos(fecha de auditoria)'
	})
