...
const credentials = JSON.parse(process.env.BQ_CM_CREDENTIALS ?? '{}')
  ...
  `
		...
		const [job] = await bigquery.createQueryJob({
			...
			params: {
				...
				end: end.toISOString().split('T').shift(),
				warehouses: warehousesUids
			}
		})
...
export async function getAbstractData(
	...
	const abstractData: Record<string, AbstractDateData> = {}
	...
	sales.forEach((sal: AbstractSale) => {
		...
		if (!dateData) {
			abstractData[dateString] = {
				...
				sales: {
					totalAmount: 0,
					totalCount: 0,
					terminals: { [terminalId]: { terminalId, totalAmount: 0, totalCount: 0 } }
				},
				purchases: {
					totalAmount: 0,
					totalCount: 0,
		...
		if (!abstractData[dateString].sales.terminals[terminalId]) {
			abstractData[dateString].sales.terminals[terminalId] = {
				terminalId,
				totalAmount: 0,
				totalCount: 0
			}
	...
	purchases.forEach((pur: PurDocuments) => {
		...
		if (!abstractData[dateString].purchases.terminals[terminalId]) {
			abstractData[dateString].purchases.terminals[terminalId] = {
				terminalId,
				totalAmount: 0,
				totalCount: 0
			}
	...
	cashClosings.forEach((cashClosing: SalCashDeskClosing) => {
		...
		if (!abstractData[dateString].cashClosings.terminals[terminalId]) {
			abstractData[dateString].cashClosings.terminals[terminalId] = {
				terminalId,
				totalAmount: 0,
				totalCount: 0
			}
	...
	orders.forEach((order: SalOrders) => {
		...
		if (!abstractData[dateString].orders.terminals[terminalId]) {
			abstractData[dateString].orders.terminals[terminalId] = {
				terminalId,
				totalAmount: 0,
				totalCount: 0
			}
...
export async function getAbstractSales(aclCompany: AclCompany, salDocsRepo: Repository<AbstractSale>) {
	const today = new Date()
	const init = new Date(today.getFullYear(), today.getMonth() - 3, 1)
	...
	sales.forEach((sal: AbstractSale) => {
		...
		if (!abstractData[dateString].terminals[terminalId]) {
			abstractData[dateString].terminals[terminalId] = {
				terminalId,
				totalAmount: 0,
				totalCount: 0
			}
...
export async function getAbstractPurchases(csmCompany: ComCompanies, csmPurchasesRepo: Repository<PurDocuments>) {
	const today = new Date()
	const init = new Date(today.getFullYear(), today.getMonth() - 3, 1)
...
export async function getAbstractExpense(aclCompany: AclCompany, expensesRepo: Repository<ExpenseEntity>) {
	const today = new Date()
	const init = new Date(today.getFullYear(), today.getMonth() - 3, 1)
...
export async function getAbstractSkusSales(warehousesUids: string[]) {
	const today = new Date()
	const init = new Date(today.getFullYear(), today.getMonth() - 3, 1)
	...
	> = Object.fromEntries(
		skus.map((s) => [s.date.value, { date: s.date.value, totalAmount: s.amount, totalCount: s.productsCount }])
	)
...
export async function getAbstractSkusPurchases(
	...
	const abstractData: {
		...
		.where('detail.created_at BETWEEN :start AND :end', {
			start: dates[0].toISOString(),
			end: new Date((dates.at(-1) as Date)?.getTime() + 86400000).toISOString()
		})
	...
	return abstractData.map((i) => ({
		date: i.date.toISOString().split('T').shift(),
